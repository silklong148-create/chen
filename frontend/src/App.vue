...
const connection = reactive({ host: '127.0.0.1', port: 3306, username: 'root', password: '', databaseType: 'MYSQL' })
const serverForm = reactive({ host: '127.0.0.1', port: 3306, username: 'root', password: '', databaseType: 'MYSQL' })
...
async function logout() {
  if (!confirm('确定退出 Mysqladmin 吗？当前 MySQL 连接将一并断开。')) return
  try { await api.logout(); authenticated.value = false; connected.value = false; currentUser.value = ''; databases.value = []; tables.value = []; activeDatabase.value = null; selectedTable.value = null; notify('已退出登录') } catch (error) { fail(error) }
}

async function connect() {
  connecting.value = true
  try { const profile = await api.connect(connection); connected.value = true; notify(`已连接 ${profile.host}:${profile.port}`); await loadDatabases() } catch (error) { fail(error) } finally { connecting.value = false }
}
...
function changeDatabaseType() {
  if (serverForm.databaseType === 'SQLSERVER' && serverForm.port === 3306) serverForm.port = 1433
  if (serverForm.databaseType === 'DAMENG' && [3306, 1433].includes(serverForm.port)) serverForm.port = 5236
  if (serverForm.databaseType === 'MYSQL' && serverForm.port === 1433) serverForm.port = 3306
}

function databaseTypeLabel(type) {
  return type === 'SQLSERVER' ? 'SQL Server' : type === 'DAMENG' ? '达梦 DM8' : 'MySQL'
}
async function switchServer() {
  connecting.value = true
  try {
    const profile = await api.connect(serverForm)
    Object.assign(connection, { host: profile.host, port: profile.port, username: profile.username, password: serverForm.password, databaseType: profile.type })
    connected.value = true; activeDatabase.value = null; selectedTable.value = null; tables.value = []; showServerDialog.value = false
    await loadDatabases(); notify(`已切换至 ${profile.host}:${profile.port}`)
...
async function loadDatabases() {
  try { databases.value = await api.databases() } catch (error) { fail(error); connected.value = false }
}
...
async function createDatabase() {
  busy.value = true
  try { await api.createDatabase(databaseForm); showDatabaseDialog.value = false; databaseForm.name = ''; await loadDatabases(); notify('数据库已创建') } catch (error) { fail(error) } finally { busy.value = false }
}
...
async function deleteDatabase(database) {
  if (!confirm(`危险操作：删除数据库 “${database.name}” 将永久清空其中所有数据表。确定删除吗？`)) return
  try { await api.deleteDatabase(database.name); if (activeDatabase.value?.name === database.name) { activeDatabase.value = null; tables.value = []; selectedTable.value = null }; await loadDatabases(); notify('数据库已删除') } catch (error) { fail(error) }
}
...
<template>
  <main v-if="!authenticated" class="connect-page auth-page">
    ...
    <section class="connect-panel" aria-labelledby="auth-title"><p class="section-label">{{ authMode === 'login' ? 'WELCOME BACK' : 'CREATE ACCOUNT' }}</p><h2 id="auth-title">{{ authMode === 'login' ? '登录 Mysqladmin' : '创建你的工作账号' }}</h2><p class="hint">{{ authMode === 'login' ? '登录后再连接需要管理的 MySQL 实例。' : '用户名使用 3–32 位字母、数字、下划线或短横线。' }}</p><form @submit.prevent="authenticate" class="connection-form"><label>用户名<input v-model.trim="authForm.username" required minlength="3" maxlength="32" autocomplete="username" placeholder="例如 admin"></label><label>密码<input v-model="authForm.password" required minlength="6" maxlength="72" type="password" autocomplete="current-password" placeholder="至少 6 位"></label><button class="primary-button" :disabled="authBusy">{{ authBusy ? '正在处理…' : authMode === 'login' ? '登录并继续' : '注册并继续' }} <span aria-hidden="true">→</span></button></form><p class="auth-switch">{{ authMode === 'login' ? '还没有账号？' : '已有账号？' }} <button @click="authMode = authMode === 'login' ? 'register' : 'login'">{{ authMode === 'login' ? '去注册' : '去登录' }}</button></p></section>
  </main>

  <main v-else class="app-shell">
    <aside class="sidebar">
      <div class="sidebar-brand"><div class="brand-mark small">M</div><span>Mysqladmin</span><small>LOCAL</small></div>
      <div :class="['connection-state', { disconnected: !connected }]"><button class="connection-card" @click="openServerManager"><span :class="['status-dot', { offline: !connected }]"></span><span><strong>{{ connected ? '已连接 · ' + (connection.databaseType === 'SQLSERVER' ? 'SQL Server' : 'MySQL') : '未连接服务端' }}</strong><small>{{ connected ? connection.host + ':' + connection.port : '请选择或切换数据库服务端' }}</small></span><span class="connection-edit">管理</span></button><button v-if="connected" class="icon-button" title="断开连接" @click="disconnect">⌁</button></div>
      <div class="sidebar-title"><span>DATABASES</span><button class="icon-button" title="创建数据库" @click="showDatabaseDialog = true">＋</button></div>
      <nav class="database-list" aria-label="数据库列表">
        <div v-for="database in userDatabases" :key="database.name" :class="['database-item', { active: activeDatabase?.name === database.name }]">
          <button @click="selectDatabase(database)"><span class="db-glyph">◈</span><span>{{ database.name }}</span></button>
          <div class="item-actions"><button title="修改数据库配置" @click="openDatabaseInfoDialog(database)">···</button><button title="删除数据库" @click="deleteDatabase(database)">×</button></div>
        </div>
        <p v-if="!connected" class="empty-side">连接服务端后显示数据库列表</p><p v-else-if="!userDatabases.length" class="empty-side">尚无可管理的业务数据库</p>
      </nav>
      <div class="sidebar-footer"><button class="user-menu" title="退出登录" @click="logout"><span class="user-avatar">{{ currentUser.slice(0, 1).toUpperCase() }}</span><span>{{ currentUser }}</span><small>退出</small></button><span class="database-total">{{ userDatabases.length }} 个数据库</span></div>
    </aside>

    <section class="workspace">
      <header class="topbar"><div class="workspace-heading"><p class="section-label">{{ !connected ? 'DATABASE SERVER' : activeDatabase ? 'DATABASE / ' + activeDatabase.name : 'MYSQL INSTANCE' }}</p><div class="heading-line"><h1>{{ !connected ? '尚未连接数据库服务端' : activeDatabase?.name || '选择一个数据库开始' }}</h1><span v-if="connected" class="server-pill"><i></i>{{ connection.databaseType === 'SQLSERVER' ? 'SQL Server' : 'MySQL' }} · {{ connection.host }}:{{ connection.port }}</span></div></div><div class="top-actions"><button class="secondary-button" :disabled="!connected" @click="loadDatabases">↻ 刷新</button><button class="primary-button" :disabled="!activeDatabase" @click="showTableDialog = true">＋ 新建数据表</button></div></header>
      ...
      <section v-else-if="!activeDatabase" class="welcome-state"><div class="welcome-symbol">◈</div><h2>选择数据库，或创建一个新的工作空间</h2><p>左侧显示当前 MySQL 实例下的业务数据库。系统数据库默认隐藏，避免误操作。</p><button class="primary-button" @click="showDatabaseDialog = true">创建数据库</button></section>

      <template v-else>
        <section class="database-summary"><div><span>字符集</span><strong>{{ activeDatabase.charset }}</strong></div><div><span>排序规则</span><strong>{{ activeDatabase.collation }}</strong></div><div><span>数据表</span><strong>{{ tables.length }}</strong></div><div><span>视图</span><strong>{{ views.length }}</strong></div><p>当前数据库的表、视图与数据预览均在此维护。</p></section>
        ...
        <section v-if="!selectedTable && !selectedView" class="content-section"><div class="section-head"><div><h2>{{ databasePanel === 'tables' ? '数据表' : '视图' }}</h2><p class="object-summary">{{ databasePanel === 'tables' ? tables.length + ' 个对象 · 选择数据表查看字段与数据' : views.length + ' 个视图 · 查看定义与查询结果' }}</p></div><button class="text-button" @click="databasePanel === 'tables' ? showTableDialog = true : openViewDialog()">{{ databasePanel === 'tables' ? '新建表 →' : '新建视图 →' }}</button></div>
          ...
          <div v-else class="empty-state"><span>{{ databasePanel === 'tables' ? '▦' : '◫' }}</span><h3>{{ databasePanel === 'tables' ? '这个数据库还是空的' : '还没有创建视图' }}</h3><p>{{ databasePanel === 'tables' ? '创建第一张表，定义字段、主键和数据类型。' : '用 SELECT 查询组合已有数据表，建立可复用的数据视图。' }}</p><button class="secondary-button" @click="databasePanel === 'tables' ? showTableDialog = true : openViewDialog()">{{ databasePanel === 'tables' ? '创建数据表' : '创建视图' }}</button></div>
        </section>
  ...
  <div v-if="showDatabaseDialog || showDatabaseInfoDialog || showTableDialog || showTableInfoDialog || showColumnDialog || showServerDialog || showViewDialog || showIndexDialog || showRowDialog" class="modal-backdrop" @click.self="showDatabaseDialog = showDatabaseInfoDialog = showTableDialog = showTableInfoDialog = showColumnDialog = showServerDialog = showViewDialog = showIndexDialog = showRowDialog = false"><section class="modal" role="dialog" aria-modal="true"><header><div><p class="section-label">{{ showServerDialog ? 'DATABASE SERVER' : showDatabaseInfoDialog ? 'DATABASE SETTINGS' : showDatabaseDialog ? 'NEW DATABASE' : showTableInfoDialog ? 'TABLE SETTINGS' : showTableDialog ? 'NEW TABLE' : showColumnDialog ? 'TABLE COLUMN' : showViewDialog ? 'DATABASE VIEW' : showIndexDialog ? 'TABLE INDEX' : 'TABLE DATA' }}</p><h2>{{ showServerDialog ? '数据库服务端' : showDatabaseInfoDialog ? '编辑数据库配置' : showDatabaseDialog ? '创建数据库' : showTableInfoDialog ? '编辑表信息' : showTableDialog ? '创建数据表' : showColumnDialog ? editingColumn ? '编辑字段' : '新增字段' : showViewDialog ? editingView ? '编辑视图' : '创建视图' : showIndexDialog ? editingIndex ? '编辑索引' : '创建索引' : editingRow ? '编辑记录' : '新增记录' }}</h2></div><button class="icon-button" @click="showDatabaseDialog = showDatabaseInfoDialog = showTableDialog = showTableInfoDialog = showColumnDialog = showServerDialog = showViewDialog = showIndexDialog = showRowDialog = false">×</button></header>
    <form v-if="showServerDialog" @submit.prevent="switchServer" class="dialog-form"><p class="server-dialog-note">选择数据库类型并填写服务端连接信息。验证成功后，系统将切换数据库列表和库表、视图、索引操作到新的实例。</p><label>数据库类型<select v-model="serverForm.databaseType" @change="changeDatabaseType"><option value="MYSQL">MySQL</option><option value="SQLSERVER">SQL Server</option><option value="DAMENG">达梦 DM8</option></select></label><label>主机地址<input v-model.trim="serverForm.host" required autocomplete="off" placeholder="127.0.0.1"></label><div class="two-fields"><label>端口<input v-model.number="serverForm.port" type="number" min="1" max="65535" required></label><label>账号<input v-model.trim="serverForm.username" required autocomplete="username"></label></div><label>密码<input v-model="serverForm.password" required type="password" autocomplete="current-password" :placeholder="serverForm.databaseType === 'SQLSERVER' ? '请输入 SQL Server 密码' : serverForm.databaseType === 'DAMENG' ? '请输入达梦账号密码' : '请输入 MySQL 密码'"></label><footer><button type="button" class="secondary-button" @click="showServerDialog = false">取消</button><button class="primary-button" :disabled="connecting">{{ connecting ? '正在验证…' : '验证并切换服务端' }}</button></footer></form>
    ...
    <form v-else-if="showDatabaseDialog" @submit.prevent="createDatabase" class="dialog-form"><label>数据库名称<input v-model.trim="databaseForm.name" required pattern="[A-Za-z0-9_$]+" placeholder="例如 app_dev"></label><div class="two-fields"><label>字符集<select v-model="databaseForm.charset"><option>utf8mb4</option><option>utf8</option></select></label><label>排序规则<select v-model="databaseForm.collation"><option>utf8mb4_unicode_ci</option><option>utf8mb4_general_ci</option></select></label></div><footer><button type="button" class="secondary-button" @click="showDatabaseDialog = false">取消</button><button class="primary-button" :disabled="busy">创建数据库</button></footer></form>
